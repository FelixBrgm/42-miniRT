...
/*   Updated: 2022/11/20 17:24:18 by pvznuzda         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
*/
// 0 - A; 1 - C; 2 - L; 3 - objects(pl, sp, cy);
int	check_identifiers(t_list *line_list)
{
	...
	{
		ft_printf("Wrong amount of identifiers.\n");
		return (-1);
	}
...
int	check_camera(char *line)
{
	...
	if (check_coords(line_split[2]) == -1) // NORMALIZED
	{
		free_split(line_split);
		return (-1);
	}
...
int	parse_input(t_data *data, int argc, char **argv)
{
	...
	if (parse_lines(data, line_list) == -1) // can return -1 meaning parser failed
		return (-2);
	free_list(&line_list);
	// display_data(*data);
	return (0);
