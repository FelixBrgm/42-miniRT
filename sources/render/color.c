...
/*   Updated: 2022/11/20 12:40:58 by fbruggem         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_color	color_calculate_light(t_data *data, t_obj_t closest)
{
	...
	hit_ratio = 1;
	light_ray = ray_create(closest.intersection,
			vector_sub(data->scene.light.position, closest.intersection));
	t_light_max = vector_length(vector_sub(data->scene.light.position,
				light_ray.origin)) / vector_length(light_ray.direction);
	light_collisions = hit_closest_obj(data, light_ray);
	if (closest.obj->sphere)
	{
		lightDir = unit_vector(vector_sub(closest.intersection, data->scene.light.position));
		normal = unit_vector(vector_sub(closest.intersection, closest.obj->sphere->position));
		hit_ratio = vector_dot(normal, vector_mul_n(lightDir, -1.0));
		if (hit_ratio < 0.0)
			hit_ratio = 0.0;
		// printf("DEGREE: %f\n", hitRatio);
	}
	...
	double distance = vector_length(vector_sub(light_ray.origin, data->scene.light.position));
	if (distance > 1000000)
		brightness = 0;
	else
		brightness = (20 - log2(distance/brightnessLessFactor)) / 20;
	// && light_collisions.t < t_light_max
	// bool isOk = vector_length(vector_sub(light_ray.origin, data->scene.light.position)) > vector_length(vector_sub(light_ray.origin, light_collisions.intersection));
	...
	if (light_collisions.t <= 0.0 || light_collisions.t > t_light_max)
	{
		// color = color_mul_n(color_add(ambient_color, color), 0.5);
		color = color_mul_n(color, data->scene.light.brightness);
		color = color_mul_n(color, (hit_ratio + data->scene.ambient.ratio) / 2);
		color = color_mul_n(color, brightness);
